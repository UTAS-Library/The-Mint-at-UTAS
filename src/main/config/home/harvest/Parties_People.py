...
class IndexData:
    ...
    def __metadata(self):
        ...
        self.utils.add(self.index, "UTAS ID", data.get("ID"))
        self.utils.add(self.index, "dc_title", "%s, %s" % (data.get("Family_Name"), data.get("Given_Name")))
        self.utils.add(self.index, "Name_with_Initial", data.get("Name_with_Initial"))
        ...
        idFields = ["ID", "URI", "NLA_Party_Identifier", "Researcher_ID", "open_ID", "Personal URI"]
        ...
        basicGroupId = data.get("Group_ID_1")
